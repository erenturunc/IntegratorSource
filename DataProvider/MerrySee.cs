    ...
    class MerrySee
    {
        public static Dictionary<string, Product> ParseXML2Products(string DataXML)
        {
            ...
            foreach (XmlNode pNode in productNodes)
            {
                ...
                p.SellingPrice = Convert.ToDouble(pNode.SelectSingleNode("price_list_campaign").InnerText.Trim(), new CultureInfo("en-US")) * (double)((double)108 / (double)100);
                if (p.SellingPrice <= 0)
                ...
                if (pNode.SelectSingleNode("subproducts") != null)
                {
                    ...
                    foreach (XmlNode varNode in variantNodes)
                    {
                        ...
                        varProduct.Attribute04 = varNode.SelectSingleNode("type1").InnerText.Trim();
                        varProduct.Attribute02 = varNode.SelectSingleNode("type2").InnerText.Trim();
